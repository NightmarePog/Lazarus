...
---
--- Grammar:
---   `public [ mut ] <identifier> [ = <expression> ]`   — an exported instance property
---   `public static <identifier> [ = <expression> ]`    — an exported class-level member
...
return StatementParser.new("PUBLIC", function(parser)
    if parser:_match("STATIC") then
        ...
        return binding.read_binding(
            ...
            "public",
            static_mut,
            "Expected member name after 'public static'",
            true
        )
    end
    if parser:_check("IDENTIFIER") and Method.looks_like_decl(parser) then
    ...
    return binding.read_binding(parser, "public", mutable, "Expected property name after 'public'")
end)
