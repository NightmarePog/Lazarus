...
describe("Codegen", function ()
    ...
    describe("control flow", function ()
        it("emits if / elseif / else", function ()
            local out = compile("static f(a, b) { if a { x = 1 } else if b { x = 2 } else { x = 3 } }")
        ...
        it("emits comparison operators", function ()
            assert.matches("a == b", compile("static f(a, b) { if a == b { x = 1 } }"))
        end)
        ...
        it("emits unary not", function ()
            assert.matches("not a", compile("static f(a) { if not a { x = 1 } }"))
        end)

        it("parenthesises a parenthesised binary operand of not", function ()
            assert.matches("not %(a == b%)", compile("static f(a, b) { if not (a == b) { x = 1 } }"))
        end)
    end)
    ...
    describe("constructor and construction", function ()
        ...
        it("lowers construction C(args) to C.new(args)", function ()
            assert.matches("p = Main%.new%(3, 4%)", members("static f() { mut p = Main(3, 4) }"))
        end)

        it("does not treat a normal call as construction", function ()
            assert.matches("Main%.helper%(%)", members("static helper() { return 0 }\nstatic f() { helper() }"))
        end)
    end)

    describe("methods (instance vs static)", function ()
        it("lowers a static method to a plain class function (no self)", function ()
            assert.equal("function Main.helper(n)\n    return n\nend",
                members("static helper(n) { return n }"))
        end)

        it("lowers an instance method with an implicit self parameter", function ()
            assert.equal("function Main.greet(self)\n    return self.name\nend",
                members("greet() { return self.name }"))
        end)

        it("prepends self before the declared parameters of an instance method", function ()
            assert.matches("function Main%.move%(self, dx, dy%)", members("move(dx, dy) { self.x = dx }"))
        end)

        it("dispatches obj.m(args) on an instance method to C.m(obj, args)", function ()
            assert.matches("Main%.m%(p, 1%)", members("m(a) { return a }\nstatic f(p) { p.m(1) }"))
        end)

        it("dispatches self.m() to C.m(self)", function ()
            assert.matches("Main%.go%(self%)", members("go() { return 0 }\nstep() { self.go() }"))
        end)

        it("leaves a field call on a non-method name as a plain obj.m() call", function ()
            assert.matches("p%.go%(%)", members("static f(p) { p.go() }"))
        end)
    end)
    ...
    describe("field access", function ()
        ...
        it("emits a compound field assignment desugared", function ()
            assert.matches("p%.x = p%.x %+ 1", members("static f(p) { p.x += 1 }"))
        end)
        ...
        it("emits a chained field read", function ()
            assert.matches("return p%.x%.y", members("static f(p) { return p.x.y }"))
        end)
    end)
    ...
    describe("type annotations are erased", function ()
        ...
        it("erases parameter and return annotations on a function", function ()
            local typed   = compile("static add(a: int, b: int): int { return a + b }")
            local untyped = compile("static add(a, b) { return a + b }")
    ...
    describe("footer (entry call)", function ()
        it("constructs and returns the instance via C.new(...) — the constructor is the entry", function ()
            local out = compile_program("constructor() {}")
